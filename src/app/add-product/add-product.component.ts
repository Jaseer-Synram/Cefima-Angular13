...
export class AddProductComponent implements OnInit {
  ...
  callspartedata(i) {
    ...
    if (i == 0) {
      ...
      sparte = this.SparteinfoForm.secondstep.value[i - 1].sparte;
    }
  ...
  setproducttype(event, index) {
    ...
    if (value == "Kein Produkttyp gefunden") {
      ...
      if ((value || "").trim()) {
        ...
        console.log("asdsadsadsadas" + JSON.stringify(this.typesOptionsArray[index]));

        let check = this.typesOptionsArray[index].find((result) => result.value === value)
        ...
        if (check) {
          ...
          this.typesOptionsArray[index].push({ id: id, value: value });
          let checkaddpt = this.addpt.find((result) => result === id)
          if (checkaddpt) {
  ...
  private _filterproducttype(value: string) {
    ...
    if (typeof value == "string") {
      ...
      if (this.producttypelist[this.lastsparteindex] != undefined) {
        ...
        const successdata = this.producttypelist[this.lastsparteindex].filter((option) =>
  ...
  handleAddressChangelandsparteadd(data, index) {
    const splitArr = data.address_components;
    this.getCountrylandsparteadd(splitArr, index);
  }
  handleAddressChangelandsparte(data, index) {
    const splitArr = data.address_components;
  ...
  getCountrylandsparte(data, index11): any {
    let that = this;
    const splitArr = data;
    // console.log(splitArr);
    splitArr.forEach(function (i, k) {
  ...
  getCountrylandsparteadd(data, index11): any {
    let that = this;
    const splitArr = data;
    // console.log(splitArr);
    splitArr.forEach(function (i, k) {
      ...
      if (content.length > 1) {
        ...
        for (let index = 0; index < content.length; index++) {
          ...
          let localityCountry = that.countryName;
          taskListArrays.controls[index11].patchValue({
            brokerage_countryOfResidence: localityCountry,
          });
  ...
  handleAllFieldssparte(data, i) {
    //console.log("original data = ", data);
    const splitArr = data.address_components;
    this.getAllFieldssparte(splitArr, i);
  }

  handleAllFieldssparteadd(data, i) {
    //console.log("original data = ", data);
    const splitArr = data.address_components;
  ...
  getAllFieldssparte(data, index11): any {
    let that = this;
    const splitArr = data;
    let str = "";
    //console.log("data", splitArr);
  ...
  getAllFieldssparteadd(data, index11): any {
    let that = this;
    const splitArr = data;
    let str = "";
    //console.log("data", splitArr);
      ...
      this.userService.uploaddocumentwithoutticketno(
        ...
        switch (event.type) {
          ...
          case HttpEventType.UploadProgress:
            ...
            this.progress1 = Math.round(event.loaded / event.total * 100);
  ...
  savedata() {
    ...
    this.userService.submitcefimacareer(data).subscribe((data) => {
